...
The [Online Help](olhelp/index.html) is available on this site and is also all built into PAMGuard so it's accessible even if you're at sea and don't have Internet. If you are having issues with the deep learning module, check out this additional [troubleshooting section](/deeplearning/deep_learning_troubleshoot.html).

The [User Tutorials](tutorials/tutorials.qmd) are also an excellent way to familiarise yourself with PAMGuard. We're hoping to expand these in the coming year.

There are also several providers of training courses in PAMGuard operation. Some of these are listed in our [links pages](links/mmo.qmd#mmo-and-pam-training), though please be aware that none of these providers are officially linked with the PAMGuard team.
...
If you think you've found a bug, please let us know by emailing [support\@pamguard.org](mailto:support@pamguard.org) including the following information.
...
The folder can be most easily accessed from the PAMGuard Help/Log File menu. Identify the correct file (note that times in the file names are in UTC) and either attach the file to an email, or copy the text into the body of an email to [support\@pamguard.org](mailto:support@pamguard.org).
