...
4\.        Bug 436. Bug in Serial Port interface was occasionally causing PAMGuard to crash without error message.
...
10\.        Add option to the Map module's Plot Overlay Options dialog, to show events occurring in the future (*look ahead*) instead of the default display of events which have already occurred. Can make it easier to match up the map with clicks seen in the Click Detector display.
...
2.       Bug 323. Difar module. Fixed a couple of small bugs in the DIFAR system which caused it to not work if old configurations were used.

3.       Bug 324. Occasional crashing in the AIS Module, as reported by user. Added warnings and more robust error handling.
...
Spelling correction on user input form (Submit - Submit)
