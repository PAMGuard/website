...
PAMGuard is currently only fully functional on Windows 64 bit computers, ideally running and Intel processor. We support limited functionality for post processing and offline analysis using Macs, but only on ones with ARM processors (Apple Silicon). PAMGuard will not run on Macs with an Intel processor.
...
Microsoft are [ending support for Windows 10](https://www.microsoft.com/en-gb/windows/end-of-support) in October 2025. For security, we are no longer allowed to operate Windows 10 machines within the University of St Andrews. Therefore new releases of PAMGuard (\> 2.02.17) will only be tested on Windows 11 computers. It is very likely that PAMGuard will continue to run under Windows 10 for many years to come, but be aware that it probably won't have been tested.

### Apple Mac

A macOS installer is now available for apple computers running macOS + with Apple Silicon (Macs with an Intel processor are not supported).
...
Note that the Mac version does not provide support for some input devices so may not be suitable for real time operation. It should however be fully functional for data post processing and offline analysis of recordings and use of the PAMGuard viewer.
