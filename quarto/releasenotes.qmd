...
Some minor bug fixes following our migration to GitHub. Note that the older Bug numbers only refer to bugs reported on the SVN site. New bugs and issues are logged on GitHub at [https://github.com/PAMGuard/PAMGuard/issues](https://github.com/PAMGuard/PAMGuard/issues). GitHub issue number are referred to with a # symbol
...
Note that updates have also been made to the Matlab interface to PAMGuard binary files, which has now also been migrated to GitHub at [https://github.com/PAMGuard/PAMGuardMatlab](https://github.com/PAMGuard/PAMGuardMatlab).
...
10\.        Add option to the Map module's Plot Overlay Options dialog, to show events occuring in the future (_look ahead_) instead of the default display of events which have already occurred. Can make it easier to match up the map with clicks seen in the Click Detector display.
...
7\.        Added ability to scale the PAMGuard display (go to _Help > Set Display Scaling Factor_ to adjust the scale). This, in conjunction with the built-in Windows Display Scaling feature, will hopefully help with issues sometimes found when using 4k/UHD/high-DPI displays. At the moment this does not fix the JavaFX components, but we're working on that.
...
If old data are opened with the PAMGuard viewer they will automatically be converted. For safety, the original binary files will not be overwritten and the new data will be placed in a new folder on your computer with the same path as the old data, but suffixed with ‘\_WithUID’, e.g. if your binary data were previously stored in the folder C:\\MySurvey\\binarydata the new data will be written to C:\\MySurvey\\binarydata\_WithUID.
...
3.       Bug 310. When exiting Viewer mode, PAMGuard queries the user whether they are sure they want to exit without saving even though they selected _Save and Exit_. Corrected, and added an _Exit without Save_ option.
...
_Click Detector RainbowClick file output_
...
Details of these bugs can be found at [https://sourceforge.net/p/PAMGuard/bugs](https://sourceforge.net/p/PAMGuard/bugs)
...
5.              The Open Office Database system has been removed from the list of available options since it is not reliable. If you require a free database solution we recommend you use the MySql Community Server [http://dev.mysql.com/downloads/mysql/](http://dev.mysql.com/downloads/mysql/)
...
The '_Hydrophone Array_' menu item moved from 'File' to 'Settings'

'_Multi Threading_' menu item moved from 'Detection' (now 'Settings') to 'File'

'_Start_' and '_Stop_' menu items removed. PAMGuard should be started and stopped using the red and black buttons.

**New Modules**

_DIFAR_ (Localisation Group)
...
_MEAL Localiser_ (Localisation Group)
...
_Accelerometer Readout_ (Sensors group)
...
_Wild ArcGIS Interface_ (Maps and Mapping group)
...
_Envelope Tracing_ (Beta Only, Sound Processing Group)
...
_Long Term Spectral Average_ (Sound Processing Group)
...
_Echo Detector and Sperm Whale IPI Computation_ (Beta only, Sound Measurements Group)
...
12.  Bug in calculated spectrum levels if data were decimated ([http://sourceforge.net/p/PAMGuard/bugs/187/](http://sourceforge.net/p/PAMGuard/bugs/187/)) . Did not affect band level calculations.
...
15.  Flickering of click display ([http://sourceforge.net/p/PAMGuard/bugs/191/](http://sourceforge.net/p/PAMGuard/bugs/191/)) now fixed.
16.  Logger forms crashing in Viewer mode. Bug now fixed.
17.  Storage options ([https://sourceforge.net/p/PAMGuard/bugs/175/](https://sourceforge.net/p/PAMGuard/bugs/175/)) whereby clicks were stored to database is now fixed.
