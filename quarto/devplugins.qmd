...
-   To develop a standard PAMGuard module refer to the [Detector for the Scottish Wild Haggis](https://github.com/PAMGuard/haggis) project.
...
Another method, which is useful if you're working on many different plugins, is to have each as a separate project within the same Eclipse workspace, then make a plugin development project which has nothing but a  copy of PamModel.java thats modified to include all of your different plugins. See [here](https://github.com/douggillespie/PluginDevelopment) for an example. At around line 1200 in the function PamModel.listPlugins I've added 14 different plugins I'm either working on or want to use from within Eclipse. 

``` java
		pluginList.add(new CollatorPlugin());
		pluginList.add(new DVPlugin());
		pluginList.add(new DummyAnalogPlugin());
		pluginList.add(new GibbonPlugin());
		pluginList.add(new TritechDaqPlugin());
		pluginList.add(new TritechDetectPlugin());
		pluginList.add(new resourceMonitorPlugin());
		pluginList.add(new RiverTriggerPlugin());
		pluginList.add(new SpermIpiPlugin());
		pluginList.add(new TrexPlugin());
		pluginList.add(new WinSoundPlugin());
		pluginList.add(new BatchJobPlugin());
		pluginList.add(new GenusPlugin());
		pluginList.add(new RIONPlugin());
```

To make it work, each project is included in the Java Build path for my plugin development project.

![Java build path for development of several plugins](images\manyplugins.png)
...
Follow these steps to bundle all files together into a single JAR, ready for distribution. Note the following are specific to the Eclipse IDE. Other IDEs will have similar functionality:
...
3.  In the window on the left, make sure the package and all necessary subfolders are selected to export (sometimes subfolders created by JavaHelp - such as JavaHelpSearch – are not included in the export list by default).
4.  Click on the box beside "Export generated class files and resources" and "Export Java source files and resources".
5.  Enter the name of the exported JAR file in the text box under "Select the export destination". You can click on the box beside "Compress the contents of the JAR file", though this is optional.
6.  The default values for the remainder of the windows can be used. Hit Finish to export the package to the location specified.
