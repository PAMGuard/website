...
If you have older binary files, the latest PAMGuard release can [update them to the latest version](https://www.pamguard.org/olhelp/utilities/BinaryStore/docs/binarystore_update.html).
:::
...
![Importing the PAMGuard R PAMBinaries library is very straightforward using the Install packages feature in RStudio](public/R_install.png).
...
![MATLAB waveform plot](public/waveform-matlab.png)
...
![R waveform plot](public/waveform-R.png)
...
![Python waveform plot](public/waveform-python.png)
...
for i = 1:length(binaryFiles)
    ...
    clicks = pgmatlab.loadPamguardBinaryFile(filePath);
end
```
...
for i = 1:length(binaryFiles)
    ...
    fprintf('Porpoise click count after %d files is %d\n', i, count);
end
```
...
::: callout-note
The variable `fileNames` is an array of file names (as strings). This allows us to open up the files one-at-a-time to prevent excessive memory usage. Note that you can use
...
folder_name = '\enter\your\path\porpoise_data\PAMBinary\20130710\'
...
In this exercise we will open another type of file: the whistle file. This differs from click data as it is produced by the [PAMGuard Whistle and Moan Detector](https://www.pamguard.org/olhelp/detectors/whistleMoanHelp/docs/whistleMoan_Overview.html). It will become quite obvious the similarities and differences between the click detector and whistle files.

::: callout-note
# Fun fact

PAMGuard has a large number of [modules](https://www.pamguard.org/coremodules.html). Each module produces different kinds of data, each of which can be read into the R, MATLAB or Python libraries for reading PAMGuard data files. This tutorial, which concerns the Click Detector and Whistle and Moan Detector, only scratches the surface of what PAMGuard, and these data file readers are capable of.
:::
...
![](./public/whistle-contour-plot-R.png)
...
::: callout-tip
Other [tutorials](https://www.pamguard.org/tutorials.html) on the PAMGuard website have step by step guides on how to use PAMGuard to generate [long term spectral averages](https://www.pamguard.org/olhelp/sound_processing/LTSA/Docs/LTSA.html).
:::
